...
- Use long-term memory tools to improve future repo work (selectively):
  ...
  - avoid saving routine progress noise, transient debugging output, or one-off branch state.
...
3. If prior repo context is likely relevant, run `memory_search` with repository identity (`owner/repo`) plus task keywords, and use only relevant hits.
...
6. Store only durable repo-specific findings with `memory_remember` and tag each entry with `repo:<owner>/<repo>`.
